...
public class OrderConsumer {
  ...
  public void consume(OrderMessage message) {
    log.info(
        "Received order from queue: orderId={}, user={}", message.orderId(), message.username());
    ...
    if (result.isPresent()) {
      ...
      String statusMessage =
          order.getStatus() == Status.ACCEPTED
              ? "Order accepted and persisted"
              : "Order rejected: validation failed";

      confirmation =
          new OrderConfirmation(
              order.getOrderId(), order.getStatus(), statusMessage, Instant.now());
    } else {
      confirmation =
          new OrderConfirmation(
              message.orderId(), Status.REJECTED, "Order processing failed", Instant.now());
    }
  ...
  private void sendConfirmation(String username, OrderConfirmation confirmation) {
    ...
    log.info(
        "Sending confirmation to user: user={}, orderId={}, status={}",
        username,
        confirmation.orderId(),
        confirmation.status());

    messagingTemplate.convertAndSendToUser(username, "/queue/order-confirmation", confirmation);
  }
}
